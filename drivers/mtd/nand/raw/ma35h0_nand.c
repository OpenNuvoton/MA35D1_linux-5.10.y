...
#define MA35H0_DRV_VERSION "20240125"
...
static int ma35h0_ooblayout_ecc(struct mtd_info *mtd, int section,
...
static int ma35h0_ooblayout_free(struct mtd_info *mtd, int section,
				  struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct nand_ecc_ctrl *ecc = &chip->ecc;
...
static const struct mtd_ooblayout_ops ma35h0_ooblayout_ops = {
	.ecc = ma35h0_ooblayout_ecc,
	.free = ma35h0_ooblayout_free,
};
...
static struct nand_ecclayout_user ma35h0_nand_oob;
...
/*
 * ma35h0_nand_hwecc_init - Initialize hardware ECC IP
 */
static void ma35h0_nand_hwecc_init(struct ma35h0_nand_info *nand)
{
	/* reset SM controller */
...
static void ma35h0_nand_initialize(struct ma35h0_nand_info *nand)
{
	/* Enable SM_EN */
...
/*--------------------------------*/
void fmiSM_CorrectData_BCH(struct ma35h0_nand_info *nand, u8 ucFieidIndex,
...
static int ma35h0_nand_correct_data(struct nand_chip *chip, u_char *dat,
...
void ma35h0_nand_enable_hwecc(struct nand_chip *chip, int mode)
{

}

/*
 * ma35h0_nand_dmac_init - Initialize dma controller
 */
static void ma35h0_nand_dmac_init(struct ma35h0_nand_info *nand)
{
	/* DMAC enable */
 ...
 */
static unsigned char ma35h0_nand_read_byte(struct nand_chip *chip)
{
	unsigned char ret;
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
 ...
 */
static void ma35h0_nand_read_buf(struct nand_chip *chip, unsigned char *buf, int len)
{
	int i;
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
...
static void ma35h0_nand_write_buf(struct nand_chip *chip, const unsigned char *buf, int len)
{
	int i;
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
...
/*
 * ma35h0_nand_dma_transfer: configer and start dma transfer
 */
static inline int ma35h0_nand_dma_transfer(struct nand_chip *chip,
...
static void ma35h0_read_buf_dma(struct nand_chip *chip, u_char *buf, int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);

	if (len == mtd->writesize) /* start transfer in DMA mode */
		ma35h0_nand_dma_transfer(chip, buf, len, 0x0);
	else {
		ma35h0_nand_read_buf(chip, buf, len);
	}
}

static void ma35h0_write_buf_dma(struct nand_chip *chip, const u_char *buf, int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);

	if (len == mtd->writesize) /* start transfer in DMA mode */
		ma35h0_nand_dma_transfer(chip, (u_char *)buf, len, 0x1);
	else
		ma35h0_nand_write_buf(chip, buf, len);
}


static int ma35h0_nand_devready(struct nand_chip *chip)
{
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
	unsigned int val;
...
static int ma35h0_waitfunc(struct nand_chip *chip)
{
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
...
static void ma35h0_nand_command(struct nand_chip *chip,
...
/* select chip */
static void ma35h0_nand_select_chip(struct nand_chip *chip, int cs)
{
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
 ...
 */
static int ma35h0_nand_calculate_ecc(struct nand_chip *chip, const u_char *dat, u_char *ecc_code)
{
	return 0;
}

static int ma35h0_nand_write_page_hwecc(struct nand_chip *chip,
	const uint8_t *buf, int oob_required, int page)
{
	...
	ma35h0_nand_command(chip, NAND_CMD_SEQIN, 0, page);
	ma35h0_nand_dma_transfer(chip, buf, mtd->writesize, 0x1);
	ma35h0_nand_command(chip, NAND_CMD_PAGEPROG, -1, -1);
	ma35h0_waitfunc(chip);
...
static int ma35h0_nand_read_page_hwecc_oob_first(struct nand_chip *chip,
	uint8_t *buf, int oob_required, int page)
{
	...
	ma35h0_nand_command(chip, NAND_CMD_READOOB, 0, page);
	ma35h0_nand_read_buf(chip, chip->oob_poi, mtd->oobsize);
	...
	if ((*(ptr+2) != 0) && (*(ptr+3) != 0))
		...
		ma35h0_nand_command(chip, NAND_CMD_READ0, 0, page);
		ma35h0_nand_dma_transfer(chip, buf, mtd->writesize, 0x0);
...
static void ma35h0_layout_oob_table(struct nand_ecclayout_user *pNandOOBTbl,
...
static irqreturn_t ma35h0_nand_irq(int irq, struct ma35h0_nand_info *nand)
{
...
static int ma35h0_nand_attach_chip(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ma35h0_nand_info *host = nand_get_controller_data(chip);
	unsigned int reg;
	...
	chip->ecc.bytes = ma35h0_nand_oob.eccbytes / chip->ecc.steps;
	chip->ecc.total = ma35h0_nand_oob.eccbytes;
	mtd_set_ooblayout(mtd, &ma35h0_ooblayout_ops);

	pr_info("attach: page %d, SMRA size %d, %d\n", mtd->writesize, mtd->oobsize, ma35h0_nand_oob.eccbytes);

	/* add mtd-id. The string should same as uboot definition */
	mtd->name = "nand0";

	ma35h0_nand_hwecc_init(host);
...
static const struct nand_controller_ops ma35h0_nand_controller_ops = {
	.attach_chip = ma35h0_nand_attach_chip,
};

static int ma35h0_nand_probe(struct platform_device *pdev)
{
	struct nand_chip *chip;
	...
	ma35h0_nand = devm_kzalloc(&pdev->dev, sizeof(struct ma35h0_nand_info), GFP_KERNEL);
	if (!ma35h0_nand)
		return -ENOMEM;

	nand_controller_init(&ma35h0_nand->controller);

	ma35h0_nand->base = devm_platform_ioremap_resource(pdev, 0);
	if (IS_ERR(ma35h0_nand->base))
		return PTR_ERR(ma35h0_nand->base);

	ma35h0_nand->pdev = pdev;
	ma35h0_nand->dev = &pdev->dev;
	chip = &ma35h0_nand->chip;
	mtd = nand_to_mtd(chip);
	nand_set_controller_data(chip, ma35h0_nand);
	...
	ma35h0_nand->clk = devm_clk_get(&pdev->dev, clkgate);
	if (IS_ERR(ma35h0_nand->clk)) {
		dev_err(&pdev->dev, "Failed to get clock\n");
			return PTR_ERR(ma35h0_nand->clk);
	}

	retval = clk_prepare_enable(ma35h0_nand->clk);
	if (retval) {
	...
	ma35h0_nand->chip.controller = &ma35h0_nand->controller;

	chip->legacy.cmdfunc     = ma35h0_nand_command;
	chip->legacy.waitfunc    = ma35h0_waitfunc;
	chip->legacy.read_byte   = ma35h0_nand_read_byte;
	chip->legacy.select_chip = ma35h0_nand_select_chip;
	chip->legacy.read_buf    = ma35h0_read_buf_dma;
	chip->legacy.write_buf   = ma35h0_write_buf_dma;
	chip->legacy.chip_delay  = 25; /* us */

	/* Check NAND device NBUSY0 pin */
	chip->legacy.dev_ready     = ma35h0_nand_devready;

	/* Read OOB data first, then HW read page */
	chip->ecc.hwctl      = ma35h0_nand_enable_hwecc;
	chip->ecc.calculate  = ma35h0_nand_calculate_ecc;
	chip->ecc.correct    = ma35h0_nand_correct_data;
	chip->ecc.write_page = ma35h0_nand_write_page_hwecc;
	chip->ecc.read_page  = ma35h0_nand_read_page_hwecc_oob_first;
	chip->ecc.read_oob   = ma35h0_nand_read_oob_hwecc;
	chip->options |= (NAND_NO_SUBPAGE_WRITE | NAND_USES_DMA);

	ma35h0_nand_initialize(ma35h0_nand);
	platform_set_drvdata(pdev, ma35h0_nand);

	ma35h0_nand->controller.ops = &ma35h0_nand_controller_ops;

	ma35h0_nand->irq = platform_get_irq(pdev, 0);
	if (ma35h0_nand->irq < 0) {
		dev_err(&pdev->dev, "failed to get platform irq\n");
		retval = -EINVAL;
		goto fail;
	}

	if (request_irq(ma35h0_nand->irq, (irq_handler_t)&ma35h0_nand_irq,
			IRQF_TRIGGER_HIGH, "ma35h0-nand", ma35h0_nand)) {
	...
	if (mtd_device_register(mtd, ma35h0_nand->parts, ma35h0_nand->nr_parts))
		goto fail;
...
fail:
	nand_cleanup(chip);
	devm_kfree(&pdev->dev, ma35h0_nand);
	return retval;
}

static int ma35h0_nand_remove(struct platform_device *pdev)
{
	struct ma35h0_nand_info *ma35h0_nand = platform_get_drvdata(pdev);
	struct nand_chip *chip = &ma35h0_nand->chip;
	int ret;
	...
	clk_disable(ma35h0_nand->clk);
	clk_put(ma35h0_nand->clk);
...
/* PM Support */
#ifdef CONFIG_PM
static int ma35h0_nand_suspend(struct platform_device *pdev, pm_message_t pm)
{
	struct ma35h0_nand_info *ma35h0_nand = platform_get_drvdata(pdev);
	unsigned long timeo = jiffies + HZ/2;
	...
	/* wait DMAC to ready */
	while (1) {
		if ((readl(ma35h0_nand->base+REG_NFI_DMACTL) & 0x200) == 0)
			break;
		if (time_after(jiffies, timeo))
			return -ETIMEDOUT;
	}
...
static int ma35h0_nand_resume(struct platform_device *pdev)
{
	struct ma35h0_nand_info *ma35h0_nand = platform_get_drvdata(pdev);

	clk_enable(ma35h0_nand->clk);
	ma35h0_nand_hwecc_init(ma35h0_nand);
	ma35h0_nand_dmac_init(ma35h0_nand);
...
#else
#define ma35h0_nand_suspend NULL
#define ma35h0_nand_resume NULL
#endif

static const struct of_device_id ma35h0_fmi_of_match[] = {
	{ .compatible = "nuvoton,ma35h0-nand" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35h0_fmi_of_match);

static struct platform_driver ma35h0_nand_driver = {
		.driver = {
		.name   = "ma35h0-nand",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(ma35h0_fmi_of_match),
		},
		.probe      = ma35h0_nand_probe,
		.remove     = ma35h0_nand_remove,
		.suspend    = ma35h0_nand_suspend,
		.resume     = ma35h0_nand_resume,
};

static int __init ma35h0_nand_init(void)
{
	int ret;

	pr_info("ma35h0 mtd nand driver version: %s\n", MA35H0_DRV_VERSION);

	ret = platform_driver_register(&ma35h0_nand_driver);
	if (ret) {
		pr_warn("nand: failed to add device driver %s\n", ma35h0_nand_driver.driver.name);
		return ret;
	}
...
static void __exit ma35h0_nand_exit(void)
{
	platform_driver_unregister(&ma35h0_nand_driver);
	pr_info("nand: unregistered successfully!\n");
}

module_init(ma35h0_nand_init);
module_exit(ma35h0_nand_exit);

MODULE_AUTHOR("nuvoton");
MODULE_DESCRIPTION("ma35h0 nand driver!");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ma35h0-nand");
