...
struct ma35h0_spi {
	...
	struct device           *dev;
	struct ma35h0_spi_info  *pdata;
	spinlock_t              lock;
	struct resource         *res;
};

static inline struct ma35h0_spi0 *to_hw(struct spi_device *sdev)
{
	return spi_master_get_devdata(sdev->master);
}

static inline void ma35h0_slave_select(struct spi_device *spi, unsigned int ssr)
{
	struct ma35h0_spi *hw = (struct ma35h0_spi *)to_hw(spi);
	unsigned int val;
...
static inline void ma35h0_spi_chipsel(struct spi_device *spi, int value)
{
	switch (value) {
	case BITBANG_CS_INACTIVE:
		ma35h0_slave_select(spi, 0);
		break;

	case BITBANG_CS_ACTIVE:
		ma35h0_slave_select(spi, 1);
		break;
	}
}

static inline void ma35h0_setup_txbitlen(struct ma35h0_spi *hw,
	unsigned int txbitlen)
{
	unsigned int val;
...
static inline void ma35h0_set_clock_polarity(struct ma35h0_spi *hw, unsigned int polarity)
{
	unsigned int val;
...
static inline void ma35h0_tx_rx_edge(struct ma35h0_spi *hw, unsigned int tx_edge, unsigned int rx_edge)
{
	unsigned int val;
...
static inline void ma35h0_send_first(struct ma35h0_spi *hw, unsigned int lsb)
{
	unsigned int val;
...
static inline void ma35h0_set_sleep(struct ma35h0_spi *hw, unsigned int sleep)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_set_divider(struct ma35h0_spi *hw)
{
	__raw_writel(hw->pdata->divider, hw->regs + REG_CLKDIV);
}

static inline void ma35h0_enable_slave(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_enable_rxth_int(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_disable_rxth_int(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_enable_txth_int(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_disable_txth_int(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35h0_enable_ssinact_int(struct ma35h0_spi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static irqreturn_t ma35h0_spi_irq(int irq, void *dev)
{
	struct ma35h0_spi *hw = dev;
	unsigned int status;
...
static int ma35h0_spi_update_state(struct spi_device *spi,
...
static int ma35h0_spi_setupxfer(struct spi_device *spi,
				struct spi_transfer *t)
{
	struct ma35h0_spi *hw = (struct ma35h0_spi *)to_hw(spi);
	unsigned long flags;
	...
	ret = ma35h0_spi_update_state(spi, t);
	if (ret)
		return ret;

	spin_lock_irqsave(&hw->lock, flags);

	ma35h0_setup_txbitlen(hw, hw->pdata->txbitlen);
	ma35h0_tx_rx_edge(hw, hw->pdata->txneg, hw->pdata->rxneg);
	ma35h0_set_clock_polarity(hw, hw->pdata->clkpol);
	ma35h0_send_first(hw, hw->pdata->lsb);
	ma35h0_set_divider(hw);
...
static int ma35h0_spi_setup(struct spi_device *spi)
{
	struct ma35h0_spi *hw = (struct ma35h0_spi *)to_hw(spi);
	int ret;

	ret = ma35h0_spi_update_state(spi, NULL);
	if (ret)
		return ret;
...
static void ma35h0_init_spi(struct ma35h0_spi *hw)
{
	unsigned long flags;
	...
	ma35h0_enable_slave(hw);
	ma35h0_enable_rxth_int(hw);
	ma35h0_enable_ssinact_int(hw);

}

static struct ma35h0_spi_info *ma35h0_spi_parse_dt(struct device *dev)
{
	struct ma35h0_spi_info *sci;
	u32 temp;
 ...
 */
static int SPI_Slave_Thread_TXRX(struct ma35h0_spi *hw)
{
	int i;
...
static int ma35h0_spi_probe(struct platform_device *pdev)
{
	struct ma35h0_spi *hw;
	struct spi_master *master;
	int err = 0;
	int status = 0;

	master = spi_alloc_master(&pdev->dev, sizeof(struct ma35h0_spi));
	if (master == NULL) {
	...
	hw->pdata = ma35h0_spi_parse_dt(&pdev->dev);
	hw->dev = &pdev->dev;
	...
	hw->bitbang.setup_transfer = ma35h0_spi_setupxfer;
	hw->bitbang.chipselect     = ma35h0_spi_chipsel;
	hw->bitbang.master->setup  = ma35h0_spi_setup;
	...
	err = request_irq(hw->irq, ma35h0_spi_irq, 0, pdev->name, hw);
	if (err) {
...
static int ma35h0_spi_remove(struct platform_device *dev)
{
	struct ma35h0_spi *hw = platform_get_drvdata(dev);
...
#ifdef CONFIG_PM
static int ma35h0_spi_suspend(struct device *dev)
{
	struct ma35h0_spi *hw = dev_get_drvdata(dev);
...
static int ma35h0_spi_resume(struct device *dev)
{
	struct ma35h0_spi *hw = dev_get_drvdata(dev);
...
static const struct dev_pm_ops ma35h0_spi_pmops = {
	.suspend    = ma35h0_spi_suspend,
	.resume     = ma35h0_spi_resume,
};

#define MA35H0_SPI_PMOPS (&ma35h0_spi_pmops)

#else
#define MA35H0_SPI_PMOPS NULL
#endif

static const struct of_device_id ma35h0_spi_slave_of_match[] = {
	{   .compatible = "nuvoton,ma35h0-spi-slave" },
	{	},
};
MODULE_DEVICE_TABLE(of, ma35h0_spi_slave_of_match);

static struct platform_driver ma35h0_spi_slave_driver = {
	.probe      = ma35h0_spi_probe,
	.remove     = ma35h0_spi_remove,
	.driver     = {
		.name   = "ma35h0-spi-slave",
		.owner  = THIS_MODULE,
		.pm	= MA35H0_SPI_PMOPS,
		.of_match_table = of_match_ptr(ma35h0_spi_slave_of_match),
	},
};
module_platform_driver(ma35h0_spi_slave_driver);

MODULE_DESCRIPTION("ma35h0 spi slave driver!");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ma35h0-spi-slave");
