...
struct ma35d0_qspi {
	...
	struct device           *dev;
	struct ma35d0_qspi_info  *pdata;
	spinlock_t              lock;
	struct resource         *res;
};

static inline struct ma35d0_qspi0 *to_hw(struct spi_device *sdev)
{
...
{
	struct ma35d0_qspi *hw = (struct ma35d0_qspi *)to_hw(spi);
	unsigned int val;
...
static inline void ma35d0_qspi_chipsel(struct spi_device *spi, int value)
{
	switch (value) {
...
static inline void ma35d0_setup_txbitlen(struct ma35d0_qspi *hw,
	unsigned int txbitlen)
{
	unsigned int val;
...
static inline void ma35d0_set_clock_polarity(struct ma35d0_qspi *hw, unsigned int polarity)
{
	unsigned int val;
...
static inline void ma35d0_tx_rx_edge(struct ma35d0_qspi *hw, unsigned int tx_edge, unsigned int rx_edge)
{
	unsigned int val;
...
static inline void ma35d0_send_first(struct ma35d0_qspi *hw, unsigned int lsb)
{
	unsigned int val;
...
static inline void ma35d0_set_sleep(struct ma35d0_qspi *hw, unsigned int sleep)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_set_divider(struct ma35d0_qspi *hw)
{
	__raw_writel(hw->pdata->divider, hw->regs + REG_CLKDIV);
}

static inline void ma35d0_enable_slave(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_enable_rxth_int(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_disable_rxth_int(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_enable_txth_int(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_disable_txth_int(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static inline void ma35d0_enable_ssinact_int(struct ma35d0_qspi *hw)
{
	unsigned int val;
	unsigned long flags;
...
static irqreturn_t ma35d0_qspi_irq(int irq, void *dev)
{
	struct ma35d0_qspi *hw = dev;
	unsigned int status;
...
static int ma35d0_qspi_update_state(struct spi_device *spi,
...
static int ma35d0_qspi_setupxfer(struct spi_device *spi,
				struct spi_transfer *t)
{
	struct ma35d0_qspi *hw = (struct ma35d0_qspi *)to_hw(spi);
	unsigned long flags;
	...
	ret = ma35d0_qspi_update_state(spi, t);
	if (ret)
		return ret;
...
static int ma35d0_qspi_setup(struct spi_device *spi)
{
	struct ma35d0_qspi *hw = (struct ma35d0_qspi *)to_hw(spi);
	int ret;

	ret = ma35d0_qspi_update_state(spi, NULL);
	if (ret)
		return ret;
...
static void ma35d0_init_spi(struct ma35d0_qspi *hw)
{
	unsigned long flags;
...
static struct ma35d0_qspi_info *ma35d0_qspi_parse_dt(struct device *dev)
{
	struct ma35d0_qspi_info *sci;
	u32 temp;
 ...
 */
static int QSPI_Slave_Thread_TXRX(struct ma35d0_qspi *hw)
{
	int i;
...
static int ma35d0_qspi_probe(struct platform_device *pdev)
{
	struct ma35d0_qspi *hw;
	struct spi_master *master;
	int err = 0;
	int status = 0;

	master = spi_alloc_master(&pdev->dev, sizeof(struct ma35d0_qspi));
	if (master == NULL) {
	...
	hw->pdata = ma35d0_qspi_parse_dt(&pdev->dev);
	hw->dev = &pdev->dev;
	...
	hw->bitbang.setup_transfer = ma35d0_qspi_setupxfer;
	hw->bitbang.chipselect     = ma35d0_qspi_chipsel;
	hw->bitbang.master->setup  = ma35d0_qspi_setup;
	...
	err = request_irq(hw->irq, ma35d0_qspi_irq, 0, pdev->name, hw);
	if (err) {
...
static int ma35d0_qspi_remove(struct platform_device *dev)
{
	struct ma35d0_qspi *hw = platform_get_drvdata(dev);
...
#ifdef CONFIG_PM
static int ma35d0_qspi_suspend(struct device *dev)
{
	struct ma35d0_qspi *hw = dev_get_drvdata(dev);
...
static int ma35d0_qspi_resume(struct device *dev)
{
	struct ma35d0_qspi *hw = dev_get_drvdata(dev);
...
static const struct dev_pm_ops ma35d0_qspi_pmops = {
	.suspend    = ma35d0_qspi_suspend,
	.resume     = ma35d0_qspi_resume,
};

#define MA35D1_QSPI_PMOPS (&ma35d0_qspi_pmops)

#else
#define MA35D1_QSPI_PMOPS NULL
#endif

static const struct of_device_id ma35d0_qspi_slave_of_match[] = {
	{   .compatible = "nuvoton,ma35d0-qspi-slave" },
	{	},
};
MODULE_DEVICE_TABLE(of, ma35d0_qspi_slave_of_match);

static struct platform_driver ma35d0_qspi_slave_driver = {
	.probe      = ma35d0_qspi_probe,
	.remove     = ma35d0_qspi_remove,
	.driver     = {
		.name   = "ma35d0-qspi-slave",
		.owner  = THIS_MODULE,
		.pm	= MA35D1_QSPI_PMOPS,
		.of_match_table = of_match_ptr(ma35d0_qspi_slave_of_match),
	},
};
module_platform_driver(ma35d0_qspi_slave_driver);

MODULE_DESCRIPTION("ma35d0 qspi slave driver!");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ma35d0-qspi-slave");
