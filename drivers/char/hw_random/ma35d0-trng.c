...
/*-------------------------------------------------------------------*/
/*  OP-TEE client driver for MA35D0 secure TRNG                     */
/*-------------------------------------------------------------------*/
/*
...
/**
 * struct optee_rng_private - OP-TEE private data of MA35D0 TRNG
 * @dev:		OP-TEE based RNG device.
 * @ctx:		OP-TEE context handler.
 * @session_id:		RNG TA session identifier.
 * @rdata_shm_pool:	Memory pool shared with OP-TEE MA35D0 TRNG
...
static int ma35d0_optee_trng_write_ks(struct optee_rng_private *pvt_data,
...
static long optee_rng_ioctl(struct hwrng *rng, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case MA35D0_TRNG_IOC_WRITE_KS:
		if (copy_from_user(&owner, argp, sizeof(owner)))
			return -EFAULT;

		if (owner != MA35D0_TRNG_KS_OWNER_AES &&
		    owner != MA35D0_TRNG_KS_OWNER_HMAC &&
		    owner != MA35D0_TRNG_KS_OWNER_ECC &&
		    owner != MA35D0_TRNG_KS_OWNER_CPU)
			return -EINVAL;

		ret = ma35d0_optee_trng_write_ks(pvt_data, owner, &key_num);
		if (ret)
			return ret;
...
static int ma35d0_optee_trng_init(struct device *dev)
{
	int ret = 0;
	struct tee_ioctl_invoke_arg inv_arg;
	struct tee_param param[4];
...
static int optee_rng_probe(struct device *dev)
{
	...
	err = ma35d0_optee_trng_init(dev);
	if (err)
		goto out_sess;
...
/*-------------------------------------------------------------------*/
/*  hwrng driver for MA35D0 TRNG                                    */
/*-------------------------------------------------------------------*/

static inline void nu_write_reg(struct ma35d0_trng *tdev, u32 val, u32 reg)
{
	writel_relaxed(val, tdev->reg_base + reg);
}

static inline u32 nu_read_reg(struct ma35d0_trng *tdev, u32 reg)
{
	return readl_relaxed(tdev->reg_base + reg);
}

static inline int ma35d0_trng_wait_busy_clear(struct ma35d0_trng *tdev)
{
...
static int ma35d0_trng_issue_command(struct ma35d0_trng *tdev, int cmd)
{
	unsigned long  timeout;
	int	err;

	err = ma35d0_trng_wait_busy_clear(tdev);
	if (err != 0)
		return err;
...
#ifdef USE_GEN_NONCE
static int ma35d0_trng_gen_nonce(struct ma35d0_trng *tdev, uint32_t *nonce)
{
	int   i, j, loop, err;
	...
	for (i = 0; i < loop; i++) {
		...
		err = ma35d0_trng_issue_command(tdev, TCMD_GEN_NONCE);
		if (err != 0)
			return err;
	}
	return 0;
...
#else
static int ma35d0_trng_gen_noise(struct ma35d0_trng *tdev)
{
	int	err;
	...
	err = ma35d0_trng_issue_command(tdev, TCMD_GEN_NOISE);
	if (err != 0)
		return err;
...
static int ma35d0_trng_create_state(struct ma35d0_trng *tdev)
{
	int	err;
	...
	err = ma35d0_trng_issue_command(tdev, TCMD_CREATE_STATE);
	if (err != 0)
		return err;
...
static int ma35d0_trng_init(struct hwrng *rng)
{
	struct ma35d0_trng *tdev = (struct ma35d0_trng *)rng->priv;
	...
	err = ma35d0_trng_gen_nonce(tdev, nonce);
#else
	pr_debug("TRNG GEN_NOISE...\n");
	err = ma35d0_trng_gen_noise(tdev);
#endif
	if (err != 0)
		return err;

	err = ma35d0_trng_create_state(tdev);
	if (err != 0)
		return err;
...
static int ma35d0_trng_read(struct hwrng *rng, void *buf,
			     size_t max, bool wait)
{
	...
	while (max >= 4) {
		...
		err = ma35d0_trng_issue_command(tdev, TCMD_GEN_RANDOM);
		if (err != 0)
			return err;
...
static long ma35d0_trng_ioctl(struct hwrng *rng, unsigned int cmd,
			      unsigned long arg)
{
	switch (cmd) {
	case MA35D0_TRNG_IOC_WRITE_KS:
...
static int ma35d0_trng_probe(struct platform_device *pdev)
{
	struct device		*dev = &pdev->dev;
	struct ma35d0_trng	*tdev;
	struct resource		*res;
	int			err;
	...
	if (optee_nuvoton) {
		pr_info("Register MA35D0 TRNG optee client driver.\n");
	...
	tdev->rng.init = ma35d0_trng_init;
	tdev->rng.read = ma35d0_trng_read;
	tdev->rng.ioctl = ma35d0_trng_ioctl;
...
static int ma35d0_trng_remove(struct platform_device *pdev)
{
...
static const struct of_device_id ma35d0_trng_dt_ids[] = {
	{ .compatible = "nuvoton,ma35d0-trng" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, ma35d0_trng_dt_ids);

static struct platform_driver ma35d0_trng_driver = {
	.probe		= ma35d0_trng_probe,
	.remove		= ma35d0_trng_remove,
	.driver		= {
		.name		= "nuvoton-trng",
		.of_match_table = ma35d0_trng_dt_ids,
	},
};

module_platform_driver(ma35d0_trng_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Nuvoton MA35D0 TRNG driver");
