...
#ifdef CONFIG_OPTEE
/*-------------------------------------------------------------------*/
/*  OP-TEE client driver for MA35H0 secure Key Store                */
/*-------------------------------------------------------------------*/
...
/**
 * struct optee_ks_private - OP-TEE private data of MA35H0 Key Store
 * @dev:		OP-TEE based device.
 * @ctx:		OP-TEE context handler.
 * @session_id:		KS TA session identifier.
 * @ks_shm_pool:	Memory pool shared with OP-TEE MA35H0 KS
...
static int ma35h0_ks_probe(struct platform_device *pdev)
{
	...
	if (optee_nuvoton) {
		pr_info("Register MA35H0 KS optee client driver.\n");
