...
#ifdef CONFIG_OPTEE
/*-------------------------------------------------------------------*/
/*  OP-TEE client driver for MA35D0 secure Key Store                */
/*-------------------------------------------------------------------*/
...
/**
 * struct optee_ks_private - OP-TEE private data of MA35D0 Key Store
 * @dev:		OP-TEE based device.
 * @ctx:		OP-TEE context handler.
 * @session_id:		KS TA session identifier.
 * @ks_shm_pool:	Memory pool shared with OP-TEE MA35D0 KS
...
/*-------------------------------------------------------------------*/
/*  Key Store driver for MA35D0 Key Store                           */
/*-------------------------------------------------------------------*/

static inline u32  nu_ks_read_reg(struct ma35d0_ks_dev *ks_dev, u32 offset)
{
...
static inline void nu_ks_write_reg(struct ma35d0_ks_dev *ks_dev,
...
static inline int ma35d0_ks_wait_busy_clear(struct ma35d0_ks_dev *ks_dev)
{
...
static int ma35d0_ks_read(struct ma35d0_ks_dev *ks_dev, void __user *arg)
{
	struct ks_read_args  r_args;
	int         err, remain_cnt;
	u32         cont_msk;
	...
	if (ma35d0_ks_wait_busy_clear(ks_dev) != 0) {
		pr_err("MA35D0 KS is busy!\n");
		return -EBUSY;
	}
...
static int ma35d0_ks_write_sram(struct ma35d0_ks_dev *ks_dev,
				 void __user *arg)
{
	struct ks_write_args  w_args;
	int         err, remain_cnt;
...
static int ma35d0_ks_write_otp(struct ma35d0_ks_dev *ks_dev, void __user *arg)
{
	struct ks_write_args  w_args;
	int         err, remain_cnt;
	u32         cont_msk;
...
static int ma35d0_ks_erase(struct ma35d0_ks_dev *ks_dev, void __user *arg)
{
	struct ks_kidx_args  k_args;
	int   err;
...
static int ma35d0_ks_erase_all(struct ma35d0_ks_dev *ks_dev)
{
	/* Just return when key store is in busy */
	if (ma35d0_ks_wait_busy_clear(ks_dev) != 0) {
		pr_err("MA35D0 KS is busy!\n");
...
static int ma35d0_ks_revoke(struct ma35d0_ks_dev *ks_dev, void __user *arg)
{
	struct ks_kidx_args  k_args;
	int   err;
...
static int ma35d0_ks_remain(struct ma35d0_ks_dev *ks_dev)
{
				 ...
				 unsigned long data)
{
	...
	switch (cmd) {
	case NU_KS_IOCTL_READ:
		rval = ma35d0_ks_read(ks_dev, argp);
		break;
	case NU_KS_IOCTL_WRITE_SRAM:
		rval = ma35d0_ks_write_sram(ks_dev, argp);
		break;
	case NU_KS_IOCTL_WRITE_OTP:
		rval = ma35d0_ks_write_otp(ks_dev, argp);
		break;
	case NU_KS_IOCTL_ERASE:
		rval = ma35d0_ks_erase(ks_dev, argp);
		break;
	case NU_KS_IOCTL_ERASE_ALL:
		rval = ma35d0_ks_erase_all(ks_dev);
		break;
	case NU_KS_IOCTL_REVOKE:
		rval = ma35d0_ks_revoke(ks_dev, argp);
		break;
	case NU_KS_IOCTL_GET_REMAIN:
		rval = ma35d0_ks_remain(ks_dev);
		break;
	default:
		/* Should not get here */
		break;
...
static const struct file_operations ma35d0_ks_fops = {
	.owner = THIS_MODULE,
	.open = ks_dev_open,
...
static int ma35d0_ks_probe(struct platform_device *pdev)
{
	struct ma35d0_ks_dev	*ks_dev;
	struct device		*dev;
	struct resource		*res;
	...
	if (optee_nuvoton) {
		pr_info("Register MA35D0 KS optee client driver.\n");
	...
	ks_dev->miscdev.fops = &ma35d0_ks_fops;
...
static int ma35d0_ks_remove(struct platform_device *pdev)
{
	struct ma35d0_ks_dev *ks_dev;
...
static const struct of_device_id ma35d0_ks_of_match[] = {
	{
		.compatible = "nuvoton,ma35d0-ks",
	},
	{ /* end of table */ }
};
MODULE_DEVICE_TABLE(of, ma35d0_ks_of_match);

static struct platform_driver ma35d0_ks_driver = {
	.probe = ma35d0_ks_probe,
	.remove =  ma35d0_ks_remove,
	.driver = {
		.name = "ma35d0-ks",
		.of_match_table = ma35d0_ks_of_match,
	},
};

module_platform_driver(ma35d0_ks_driver);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("Nuvoton, Inc");
MODULE_DESCRIPTION("Nuvoton MA35D0 Key Store Driver");
