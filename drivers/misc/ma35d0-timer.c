...
#include <uapi/misc/ma35d0_timer.h>
#include "regs-ma35d0-timer.h"
...
static struct ma35d0_timer *tmr[TIMER_CH];


static u8 gu8_ch;
static u32 gu32_cnt;

static irqreturn_t ma35d0_timer_interrupt(int irq, void *dev_id)
{
	struct ma35d0_timer *t = (struct ma35d0_timer *)dev_id;
	static int cnt = 0;
...
static void timer_SwitchClkSrc(u8 u8clksel, struct ma35d0_timer *t)
{
	struct clk *clkmux;
	u8 ch;
	u32 val, tmpval;
...
static void stop_timer(struct ma35d0_timer *t)
{
	unsigned long flag;
...
{
	unsigned long flag;
	struct ma35d0_timer *t = (struct ma35d0_timer *)filp->private_data;
	int ret = 0;
...
static int timer_open(struct inode *inode, struct file *filp)
{
	...
	if (request_irq(tmr[ch]->irq, ma35d0_timer_interrupt,
			IRQF_NO_SUSPEND, "ma35d0-timer", tmr[ch])) {
...
static int ma35d0_timer_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "Nuvoton MA35D0 Timer Driver");
	...
	ch = val32[0];
	tmr[ch] = devm_kzalloc(&pdev->dev, sizeof(struct ma35d0_timer), GFP_KERNEL);
	if (tmr[ch] == NULL) {
...
static int ma35d0_timer_remove(struct platform_device *pdev)
{
	struct ma35d0_timer *t = platform_get_drvdata(pdev);
	u8 ch = t->ch;
...
static int ma35d0_timer_suspend(struct platform_device *pdev,
				pm_message_t state)
{
	struct ma35d0_timer *t = platform_get_drvdata(pdev);
...
static int ma35d0_timer_resume(struct platform_device *pdev)
{
	struct ma35d0_timer *t = platform_get_drvdata(pdev);
...
static const struct of_device_id ma35d0_tmr_of_match[] = {
	{ .compatible = "nuvoton,ma35d0-timer" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35d0_tmr_of_match);

static struct platform_driver ma35d0_tmr_driver = {
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "ma35d0-timer",
		.of_match_table = of_match_ptr(ma35d0_tmr_of_match),
	},
	.probe		= ma35d0_timer_probe,
	.remove		= ma35d0_timer_remove,
	.suspend	= ma35d0_timer_suspend,
	.resume		= ma35d0_timer_resume,
};

module_platform_driver(ma35d0_tmr_driver);

MODULE_ALIAS("platform:ma35d0-timer");
MODULE_DESCRIPTION("Timer driver for Nuvoton MA35D0");
MODULE_LICENSE("GPL");
