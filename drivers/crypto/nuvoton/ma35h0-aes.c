...
/*
 * linux/driver/crypto/nuvoton/ma35h0-aes.c
 *
...
static int ma35h0_aes_dma_cascade(struct nu_aes_dev *dd, int err);
...
static struct nu_aes_dev *ma35h0_aes_find_dev(struct nu_aes_base_ctx *ctx)
{
...
static int ma35h0_aes_sg_to_buffer(struct nu_aes_dev *dd, u8 *bptr, int max_cnt)
{
	int	in_cnt, copy_len;
...
static int ma35h0_aes_buffer_to_sg(struct nu_aes_dev *dd, u8 *bptr, int max_cnt)
{
...
static int ma35h0_aes_get_output(struct nu_aes_dev *dd)
{
	struct nu_aes_base_ctx  *ctx = dd->ctx;
	int     retval;

	if ((ctx->mode & AES_CTL_OPMODE_MASK) == AES_MODE_GCM) {
		ma35h0_aes_buffer_to_sg(dd, dd->inbuf, ctx->assoclen);

		retval = ma35h0_aes_buffer_to_sg(dd, dd->outbuf, ctx->text_len);
		if ((retval % 16) != 0) {
		...
		ma35h0_aes_buffer_to_sg(dd, dd->outbuf + retval, ctx->authsize);
		...
		retval = ma35h0_aes_buffer_to_sg(dd, dd->outbuf, dd->dma_len);
		dd->dma_len = 0;
	}
	return 0;
}

static int ma35h0_aes_complete(struct nu_aes_dev *dd, int err)
{
...
static int ma35h0_aes_dma_run(struct nu_aes_dev *dd, u32 cascade)
{
	...
	dd->dma_len += ma35h0_aes_sg_to_buffer(dd, dd->inbuf + dd->dma_len,
					       AES_BUFF_SIZE - dd->dma_len);

	if (!dd->in_sg || (dd->req_len == 0)) {
		dd->resume = ma35h0_aes_complete;	/* no more data */
		dma_ctl = cascade | AES_CTL_DMALAST;
	} else {
		dd->resume = ma35h0_aes_dma_cascade;
		dma_ctl = cascade;
	}
...
static int ma35h0_aes_dma_cascade(struct nu_aes_dev *dd, int err)
{
	...
	ma35h0_aes_get_output(dd);

	/* cascade AES DMA to process remaining data */
	return ma35h0_aes_dma_run(dd, AES_CTL_DMACSCAD);
}

static int ma35h0_aes_dma_start(struct nu_aes_dev *dd, int err)
{
	struct skcipher_request *req = skcipher_request_cast(dd->areq);
	struct nu_aes_base_ctx  *ctx = dd->ctx;
	u32 *iv = (u32 *)req->iv;
	...
	if ((req->cryptlen == 0) || (req->src == NULL) || (req->dst == NULL))
		return ma35h0_aes_complete(dd, 0);  /* no data */
	...
	return ma35h0_aes_dma_run(dd, 0);
}

static int ma35h0_aes_handle_queue(struct nu_aes_dev *dd,
...
static int ma35h0_aes_crypt(struct skcipher_request *req, u32 mode)
{
	...
	aes_dd = ma35h0_aes_find_dev(ctx);
	if (!aes_dd)
		return -ENODEV;
	...
	return ma35h0_aes_handle_queue(aes_dd, &req->base);
}

static int ma35h0_aes_setkey(struct crypto_skcipher *tfm, const u8 *key,
			     unsigned int keylen)
{
...
static int ma35h0_aes_ecb_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_ECB | AES_CTL_ENCRPT);
}

static int ma35h0_aes_ecb_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_ECB);
}

static int ma35h0_aes_cbc_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CBC | AES_CTL_ENCRPT);
}

static int ma35h0_aes_cbc_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CBC);
}

static int ma35h0_aes_cfb_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CFB | AES_CTL_ENCRPT);
}

static int ma35h0_aes_cfb_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CFB);
}

static int ma35h0_aes_ofb_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_OFB | AES_CTL_ENCRPT);
}

static int ma35h0_aes_ofb_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_OFB);
}

static int ma35h0_aes_ctr_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CTR | AES_CTL_ENCRPT);
}

static int ma35h0_aes_ctr_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_MODE_CTR);
}

#ifdef SUPPORT_SM4
static int ma35h0_sm4_ecb_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN | AES_MODE_ECB | AES_CTL_ENCRPT);
}

static int ma35h0_sm4_ecb_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN | AES_MODE_ECB);
}

static int ma35h0_sm4_cbc_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN | AES_MODE_CBC | AES_CTL_ENCRPT);
}

static int ma35h0_sm4_cbc_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN | AES_MODE_CBC);
}

static int ma35h0_sm4_ctr_encrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN | AES_MODE_CTR | AES_CTL_ENCRPT);
}

static int ma35h0_sm4_ctr_decrypt(struct skcipher_request *req)
{
	return ma35h0_aes_crypt(req, AES_CTL_SM4EN|AES_MODE_CTR);
}
#endif

static int ma35h0_aes_cra_init(struct crypto_tfm *tfm)
{
	struct nu_aes_ctx *ctx = crypto_tfm_ctx(tfm);
	struct nu_aes_dev  *aes_dd;

	// printk("AES: %s\n", tfm->__crt_alg->cra_driver_name);
	ctx->base.start = ma35h0_aes_dma_start;

	aes_dd = ma35h0_aes_find_dev(&ctx->base);
	if (!aes_dd)
		return -ENODEV;
...
static void ma35h0_aes_cra_exit(struct crypto_tfm *tfm)
{
}

static struct skcipher_alg ma35h0_aes_algs[] = {
{
	.base.cra_name		= "cbc(aes)",
	.base.cra_driver_name	= "ma35h0-cbc-aes",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	.base.cra_alignmask	= 0xf,
	.base.cra_init		= ma35h0_aes_cra_init,
	.base.cra_exit		= ma35h0_aes_cra_exit,
	.base.cra_module	= THIS_MODULE,

	.min_keysize		= AES_MIN_KEY_SIZE,
	.max_keysize		= AES_MAX_KEY_SIZE,
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_aes_cbc_encrypt,
	.decrypt		= ma35h0_aes_cbc_decrypt,
},
{
	.base.cra_name		= "ecb(aes)",
	.base.cra_driver_name	= "ma35h0-ecb-aes",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.max_keysize		= AES_MAX_KEY_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_aes_ecb_encrypt,
	.decrypt		= ma35h0_aes_ecb_decrypt,
},
{
	.base.cra_name		= "cfb(aes)",
	.base.cra_driver_name	= "ma35h0-cfb-aes",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_aes_cfb_encrypt,
	.decrypt		= ma35h0_aes_cfb_decrypt,
},
{
	.base.cra_name		= "ofb(aes)",
	.base.cra_driver_name	= "ma35h0-ofb-aes",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_aes_ofb_encrypt,
	.decrypt		= ma35h0_aes_ofb_decrypt,
},
{
	.base.cra_name		= "ctr(aes)",
	.base.cra_driver_name	= "ma35h0-ctr-aes",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	...
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_aes_ctr_encrypt,
	.decrypt		= ma35h0_aes_ctr_decrypt,
},
#ifdef SUPPORT_SM4
{
	.base.cra_name		= "ecb(sm4)",
	.base.cra_driver_name	= "ma35h0-ecb-sm4",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.max_keysize		= AES_MAX_KEY_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_sm4_ecb_encrypt,
	.decrypt		= ma35h0_sm4_ecb_decrypt,
},
{
	.base.cra_name		= "cbc(sm4)",
	.base.cra_driver_name	= "ma35h0-cbc-sm4",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= AES_BLOCK_SIZE,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_sm4_cbc_encrypt,
	.decrypt		= ma35h0_sm4_cbc_decrypt,
},
{
	.base.cra_name		= "ctr(sm4)",
	.base.cra_driver_name	= "ma35h0-ctr-sm4",
	.base.cra_priority	= 400,
	.base.cra_flags		= CRYPTO_ALG_ASYNC,
	.base.cra_blocksize	= 1,
	.base.cra_ctxsize	= sizeof(struct nu_aes_ctx),
	...
	.ivsize			= AES_BLOCK_SIZE,
	.setkey			= ma35h0_aes_setkey,
	.encrypt		= ma35h0_sm4_ctr_encrypt,
	.decrypt		= ma35h0_sm4_ctr_decrypt,
},
#endif
};   /* ma35h0_aes_algs */
...
static int ma35h0_aes_gcm_dma_start(struct nu_aes_dev *dd, int err)
{
...
static int ma35h0_aes_gcm_crypt(struct aead_request *req, u32 mode)
{
...
static int ma35h0_aes_gcm_encrypt(struct aead_request *req)
{
	return ma35h0_aes_gcm_crypt(req, AES_CTL_ENCRPT);
}

static int ma35h0_aes_gcm_decrypt(struct aead_request *req)
{
	return ma35h0_aes_gcm_crypt(req, 0);
}

static int ma35h0_aes_gcm_setkey(struct crypto_aead *tfm,
...
static int ma35h0_aes_gcm_setauthsize(struct crypto_aead *aead, u32 authsize)
{
...
static int ma35h0_aes_gcm_init(struct crypto_aead *aead)
{
	struct nu_aes_ctx *ctx = crypto_aead_ctx(aead);
	struct nu_aes_dev  *aes_dd;

	ctx->base.start = ma35h0_aes_gcm_dma_start;
...
static void ma35h0_aes_gcm_exit(struct crypto_aead *aead)
{
#ifdef CONFIG_OPTEE
	struct nu_aes_ctx *ctx = crypto_aead_ctx(aead);
	struct nu_aes_dev  *aes_dd;
...
static struct aead_alg  ma35h0_aes_gcm_alg[] = {
{
	.setkey		= ma35h0_aes_gcm_setkey,
	.setauthsize	= ma35h0_aes_gcm_setauthsize,
	.encrypt	= ma35h0_aes_gcm_encrypt,
	.decrypt	= ma35h0_aes_gcm_decrypt,
	.init		= ma35h0_aes_gcm_init,
	.exit		= ma35h0_aes_gcm_exit,
	.ivsize		= GCM_AES_IV_SIZE,
	.maxauthsize	= AES_BLOCK_SIZE,
	.base = {
		.cra_name		= "gcm(aes)",
		.cra_driver_name	= "ma35h0-gcm-aes",
		.cra_priority		= 400,
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
static int ma35h0_aes_ccm_dma_start(struct nu_aes_dev *dd, int err)
{
	struct aead_request *req = aead_request_cast(dd->areq);
	struct nu_aes_base_ctx  *ctx = dd->ctx;
	u8	*b;
	...
	if (alen > 0) {
		...
		ma35h0_aes_sg_to_buffer(dd, b + 2, use_len);
		...
		while (len_left > 0) {
			...
			memset(b, 0, 16);
			ma35h0_aes_sg_to_buffer(dd, b, use_len);
			b += 16;
			len_left -= use_len;
		}
	}
	 ...
	 */
	if (!(ctx->mode & AES_CTL_ENCRPT)) {
		q = ma35h0_aes_sg_to_buffer(dd, ctx->tag,
...
static int ma35h0_aes_ccm_crypt(struct aead_request *req, u32 mode)
{
	struct nu_aes_base_ctx	*ctx = crypto_aead_ctx(crypto_aead_reqtfm(req));
	struct nu_aes_dev	*aes_dd;
...
static int ma35h0_aes_ccm_encrypt(struct aead_request *req)
{
	return ma35h0_aes_ccm_crypt(req, AES_CTL_ENCRPT);
}

static int ma35h0_aes_ccm_decrypt(struct aead_request *req)
{
	return ma35h0_aes_ccm_crypt(req, 0);
}

static int ma35h0_aes_ccm_setauthsize(struct crypto_aead *aead, u32 authsize)
{
	struct nu_aes_base_ctx *ctx = crypto_aead_ctx(aead);
...
static int ma35h0_aes_ccm_init(struct crypto_aead *aead)
{
	struct nu_aes_ctx *ctx = crypto_aead_ctx(aead);
	struct nu_aes_dev  *aes_dd;

	ctx->base.start = ma35h0_aes_ccm_dma_start;
...
static void ma35h0_aes_ccm_exit(struct crypto_aead *aead)
{
#ifdef CONFIG_OPTEE
	struct nu_aes_ctx *ctx = crypto_aead_ctx(aead);
	struct nu_aes_dev  *aes_dd;
...
static struct aead_alg  ma35h0_aes_ccm_alg[] = {
{
	.setkey		= ma35h0_aes_gcm_setkey,  /* same routine as GCM */
	.setauthsize	= ma35h0_aes_ccm_setauthsize,
	.encrypt	= ma35h0_aes_ccm_encrypt,
	.decrypt	= ma35h0_aes_ccm_decrypt,
	.init		= ma35h0_aes_ccm_init,
	.exit		= ma35h0_aes_ccm_exit,
	.ivsize		= AES_BLOCK_SIZE,
	.maxauthsize	= AES_BLOCK_SIZE,
	.base = {
		.cra_name		= "ccm(aes)",
		.cra_driver_name	= "ma35h0-ccm-aes",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= 1,
		.cra_ctxsize		= sizeof(struct nu_aes_ctx),
...
static void ma35h0_aes_queue_task(unsigned long data)
{
	struct nu_aes_dev *dd = (struct nu_aes_dev *)data;

	ma35h0_aes_handle_queue(dd, NULL);
}

static void ma35h0_aes_done_task(unsigned long data)
{
	struct nu_aes_dev *dd = (struct nu_aes_dev *)data;
...
static int ma35h0_register_gcm_ccm(struct device *dev)
{
	int i, err;
	 ...
	 */
	err = crypto_register_aeads(ma35h0_aes_gcm_alg, ARRAY_SIZE(ma35h0_aes_gcm_alg));
	if (err) {
		for (i = 0; i < ARRAY_SIZE(ma35h0_aes_algs); i++)
			crypto_unregister_skcipher(&ma35h0_aes_algs[i]);
		dev_err(dev, "Could not register ma35h0_aes_gcm_algs!\n");
		return err;
	}
	 ...
	 */
	err = crypto_register_aeads(ma35h0_aes_ccm_alg, ARRAY_SIZE(ma35h0_aes_ccm_alg));
	if (err) {
		for (i = 0; i < ARRAY_SIZE(ma35h0_aes_algs); i++)
			crypto_unregister_skcipher(&ma35h0_aes_algs[i]);
		crypto_unregister_aeads(ma35h0_aes_gcm_alg, ARRAY_SIZE(ma35h0_aes_gcm_alg));
		dev_err(dev, "Could not register ma35h0_aes_ccm_algs!\n");
		return err;
	}
	return 0;
}

int ma35h0_aes_probe(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	...
	tasklet_init(&aes_dd->done_task, ma35h0_aes_done_task, (unsigned long)aes_dd);
	tasklet_init(&aes_dd->queue_task, ma35h0_aes_queue_task, (unsigned long)aes_dd);
	 ...
	 */
	for (i = 0; i < ARRAY_SIZE(ma35h0_aes_algs); i++) {
		err = crypto_register_skcipher(&ma35h0_aes_algs[i]);
		if (err) {
			dev_err(dev, "Could not register ma35h0_aes_algs!\n");
			err = -EINVAL;
			goto err_algs;
		}
	}
	...
	if (nu_cryp_dev->use_optee == false) {
		err = ma35h0_register_gcm_ccm(dev);
		if (err)
			goto err_algs;
	}
...
int ma35h0_aes_remove(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	struct nu_aes_dev  *aes_dd = &nu_cryp_dev->aes_dd;
	int i;
	...
	crypto_unregister_aeads(ma35h0_aes_gcm_alg, ARRAY_SIZE(ma35h0_aes_gcm_alg));
	crypto_unregister_aeads(ma35h0_aes_ccm_alg, ARRAY_SIZE(ma35h0_aes_ccm_alg));
