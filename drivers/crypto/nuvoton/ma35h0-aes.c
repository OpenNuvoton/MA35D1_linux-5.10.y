...
static int ma35h0_aes_sg_to_buffer(struct nu_aes_dev *dd, u8 *bptr,
				    int max_cnt)
{
	int in_cnt, copy_len;
...
static int ma35h0_aes_buffer_to_sg(struct nu_aes_dev *dd, u8 *bptr,
				    int max_cnt)
{
...
static int ma35h0_aes_handle_queue(struct nu_aes_dev *dd, struct crypto_async_request *new_areq)
{
...
static int ma35h0_aes_gcm_dma_start(struct nu_aes_dev *dd, int err)
{
	 ...
	 */
	if (req->assoclen) {
		dd->dma_len += ma35h0_aes_sg_to_buffer(dd, dd->inbuf +
...
int ma35h0_aes_probe(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	...
	pr_info("MA35H0 Crypto AES engine enabled.\n");
	return 0;
