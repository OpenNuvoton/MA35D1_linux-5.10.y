...
/*
 * linux/driver/crypto/nuvoton/ma35h0-rsa.c
 *
...
static struct nu_rsa_dev *ma35h0_rsa_find_dev(struct nu_rsa_ctx *ctx)
{
...
static int ma35h0_rsa_sg_to_buffer(struct scatterlist *sgl,
				    u8 *buff, int max_len)
{
...
static int ma35h0_rsa_buffer_to_sg(u8 *buff, int data_cnt,
...
static int ma35h0_rsa_enc(struct akcipher_request *req)
{
	struct crypto_akcipher *tfm = crypto_akcipher_reqtfm(req);
	struct nu_rsa_ctx *ctx = akcipher_tfm_ctx(tfm);
	struct nu_rsa_dev *dd = ma35h0_rsa_find_dev(ctx);
	...
	len = ma35h0_rsa_sg_to_buffer(req->src, m,
	...
	ma35h0_rsa_buffer_to_sg(m, min_t(int, len, (int)req->dst_len), req->dst);
	return err;
}

static int ma35h0_rsa_dec(struct akcipher_request *req)
{
	struct crypto_akcipher *tfm = crypto_akcipher_reqtfm(req);
	struct nu_rsa_ctx *ctx = akcipher_tfm_ctx(tfm);
	struct nu_rsa_dev *dd = ma35h0_rsa_find_dev(ctx);
...
static int ma35h0_rsa_set_priv_key(struct crypto_akcipher *tfm,
...
static unsigned int ma35h0_rsa_max_size(struct crypto_akcipher *tfm)
{
	struct nu_rsa_ctx  *ctx = akcipher_tfm_ctx(tfm);
...
static int ma35h0_rsa_init_tfm(struct crypto_akcipher *tfm)
{
	struct nu_rsa_ctx  *ctx = akcipher_tfm_ctx(tfm);
...
static struct akcipher_alg  ma35h0_rsa = {
	.encrypt = ma35h0_rsa_enc,
	.decrypt = ma35h0_rsa_dec,
	.set_priv_key = ma35h0_rsa_set_priv_key,
	.set_pub_key = ma35h0_rsa_set_pub_key,
	.max_size = ma35h0_rsa_max_size,
	.init = ma35h0_rsa_init_tfm,
	.base = {
		.cra_name = "rsa",
		.cra_driver_name = "rsa-ma35h0",
		.cra_priority = 500,
...
int ma35h0_rsa_probe(struct device *dev,
		      struct nu_crypto_dev *nu_cryp_dev)
{
	...
	if (nu_cryp_dev->rsa_ioctl == true) {
		...
		err = crypto_register_akcipher(&ma35h0_rsa);
		if (err) {
			pr_err("[%s] - failed to register ma35h0_rsa! %d\n",
				__func__, err);
			goto err_out;
		}
	}
	pr_info("MA35H0 Crypto RSA engine enabled.\n");
	return 0;
...
int ma35h0_rsa_remove(struct device *dev,
		       struct nu_crypto_dev *nu_cryp_dev)
{
	struct nu_rsa_dev  *rsa_dd = &nu_cryp_dev->rsa_dd;
	...
	if (nu_cryp_dev->rsa_ioctl == true)
		...
		crypto_unregister_akcipher(&ma35h0_rsa);
