...
/*
 * linux/driver/crypto/nuvoton/ma35d0-ecc.c
 *
...
static struct nu_ecc_dev *ma35d0_ecc_find_dev(struct nu_ecc_ctx *tctx)
{
...
static inline int ma35d0_wait_ecc_complete(struct nu_ecc_dev *dd, int timeout)
{
	unsigned long   t_out;
...
static int ma35d0_ecc_init_curve(int curve_id, struct nu_ecc_ctx *ctx)
{
	struct nu_ecc_dev *dd = ctx->dd;
...
static int ma35d0_ecc_point_mult(struct nu_ecc_ctx *ctx, u8 *private_key,
				 u8 *public_key, u8 *result)
{
	...
	if (dd->nu_cdev->use_optee == false) {
		err = ma35d0_wait_ecc_complete(dd, 2000);
		if (err)
			return err;
	} else {
		/*
...
static int ma35d0_ecdh_set_secret(struct crypto_kpp *tfm, const void *buf,
				  unsigned int len)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
	struct nu_ecc_dev *dd = ma35d0_ecc_find_dev(ctx);
	struct ecdh params;
	...
	if (params.curve_id == ECC_CURVE_NIST_P192) {
		ndigits = 3;
		err = ma35d0_ecc_init_curve(CURVE_P_192, ctx);
	} else if (params.curve_id == ECC_CURVE_NIST_P256) {
		ndigits = 4;
		err = ma35d0_ecc_init_curve(CURVE_P_256, ctx);
	} else {
...
static int ma35d0_ecdh_compute_value(struct kpp_request *req)
{
	...
	if (req->src) {
		...
		ret = ma35d0_ecc_point_mult(ctx, ctx->private_key, public_key, shared_secret);

		buf = shared_secret;
	} else {
		/* Make public key by private key and G */
		ret = ma35d0_ecc_point_mult(ctx, ctx->private_key, NULL, public_key);
		buf = public_key;
...
static unsigned int ma35d0_ecdh_max_size(struct crypto_kpp *tfm)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
...
static struct kpp_alg ma35d0_ecdh = {
	.set_secret = ma35d0_ecdh_set_secret,
	.generate_public_key = ma35d0_ecdh_compute_value,
	.compute_shared_secret = ma35d0_ecdh_compute_value,
	.max_size = ma35d0_ecdh_max_size,
	.base = {
...
static long nvt_ecc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ECC_IOC_SET_CURVE:
		...
		return ma35d0_ecc_init_curve(arg, ecc_ctx);
	...
	case ECC_IOC_POINT_MUL:
		...
		if (use_optee == false) {
			ret = ma35d0_wait_ecc_complete(dd, 2000);
			if (ret)
				return ret;
		} else {
			/*
...
int ma35d0_ecc_probe(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	...
	if (nu_cryp_dev->ecc_ioctl == true) {
		...
		err = crypto_register_kpp(&ma35d0_ecdh);
		if (err) {
	...
	pr_info("MA35D0 Crypto ECC engine enabled.\n");
	return 0;
...
err_out:
	...
	dev_err(dev, "MA35D0 ECC initialization failed. %d\n", err);
	return err;
}

int ma35d0_ecc_remove(struct device *dev,
			   struct nu_crypto_dev *nu_cryp_dev)
{
	...
	if (nu_cryp_dev->ecc_ioctl == true)
		...
		crypto_unregister_kpp(&ma35d0_ecdh);
