...
/*
 * linux/driver/crypto/nuvoton/ma35h0-crypto.c
 *
...
static struct nu_crypto_dev *_ma35h0_crypto_device;
...
static int optee_crypto_probe(struct device *dev)
{
	...
	nu_cryp_dev = _ma35h0_crypto_device;
...
int ma35h0_crypto_optee_init(struct nu_crypto_dev *nu_cryp_dev)
{
	int err;
	...
	pr_info("Register MA35H0 Crypto optee client driver.\n");
...
static irqreturn_t ma35h0_crypto_irq(int irq, void *data)
{
	struct nu_crypto_dev  *nu_cryp_dev =
...
static int ma35h0_crypto_probe(struct platform_device *pdev)
{
	...
	/* MA35H0 crypto engine clock should be enabled by firmware. */
	nu_cryp_dev = devm_kzalloc(dev, sizeof(*nu_cryp_dev), GFP_KERNEL);
	if (!nu_cryp_dev)
		return -ENOMEM;

	dev_set_drvdata(dev, nu_cryp_dev);
	_ma35h0_crypto_device = nu_cryp_dev;
	...
	if (!nu_cryp_dev->use_optee) {
		...
		err = devm_request_irq(dev, irq, ma35h0_crypto_irq, IRQF_SHARED,
				       "ma35h0-crypto", nu_cryp_dev);
		if (err) {
			dev_err(dev, "Failed to request IRQ%d: err: %d.\n", irq, err);
			return err;
		}

		err = ma35h0_prng_probe(dev, nu_cryp_dev->reg_base, &nu_cryp_dev->prng);
		if (err)
			dev_err(dev, "failed to init PRNG!\n");
	}

	err = ma35h0_aes_probe(dev, nu_cryp_dev);
	if (err)
		dev_err(dev, "failed to init AES!\n");

	err = ma35h0_sha_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35h0_ecc_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35h0_rsa_probe(dev, nu_cryp_dev);
	if (err)
...
static int ma35h0_crypto_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct nu_crypto_dev *nu_cryp_dev;
	...
	ma35h0_prng_remove(&pdev->dev, nu_cryp_dev->prng);
	ma35h0_aes_remove(&pdev->dev, nu_cryp_dev);
	ma35h0_sha_remove(&pdev->dev, nu_cryp_dev);

#ifdef CONFIG_CRYPTO_ECDH
	ma35h0_ecc_remove(&pdev->dev, nu_cryp_dev);
#endif

#ifdef CONFIG_CRYPTO_RSA
	ma35h0_rsa_remove(&pdev->dev, nu_cryp_dev);
#endif
	devm_kfree(dev, nu_cryp_dev);
	return 0;
}

static const struct of_device_id ma35h0_crypto_of_match[] = {
	{ .compatible = "nuvoton,ma35h0-crypto" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35h0_crypto_of_match);


static struct platform_driver ma35h0_crypto_driver = {
	.probe  = ma35h0_crypto_probe,
	.remove = ma35h0_crypto_remove,
	.driver = {
		.name   = "ma35h0-crypto",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(ma35h0_crypto_of_match),
	},
};

module_platform_driver(ma35h0_crypto_driver);
