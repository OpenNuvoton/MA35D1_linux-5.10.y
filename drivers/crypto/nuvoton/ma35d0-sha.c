...
/*
 * linux/driver/crypto/nuvoton/ma35d0-sha.c
 *
...
static struct nu_sha_dev *ma35d0_sha_find_dev(struct nu_sha_ctx *tctx)
{
 ...
 */
static void  ma35d0_sha_get_result(struct ahash_request *req)
{
 ...
 */
static void ma35d0_sha_finish_req(struct nu_sha_reqctx *ctx, int err)
{
...
static int ma35d0_sha_init(struct ahash_request *req)
{
...
static void ma35d0_sha_sg_to_dma_buffer(struct ahash_request *req, struct nu_sha_reqctx *ctx)
{
	int	copy_len;
...
static int ma35d0_sha_update_start(struct nu_sha_dev *dd)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	int err = 0;

	if ((ctx->req_len > 0) &&  (ctx->bufcnt < ctx->dma_max_size))
		ma35d0_sha_sg_to_dma_buffer(dd->req, ctx);
	...
	if (ctx->flags & SHA_FLAGS_KEY_BLK) {
		if ((ctx->flags & (SHA_FLAGS_FINUP | SHA_FLAGS_FINAL)) &&
			...
			ma35d0_sha_finish_req(ctx, -EINVAL);
			return -EINVAL;
		}
		err = ma35d0_sha_dma_run(dd, 1);
		if (err != -EINPROGRESS) {
			/* DMA trigger failed, abort! */
			ma35d0_sha_finish_req(ctx, err);
		}
		 ...
		 */
		ma35d0_sha_finish_req(ctx, 0);
		err = 0;
	}
	return err;
}

static int ma35d0_sha_handle_queue(struct nu_sha_dev *dd,
				    struct ahash_request *req)
{
	...
	return ma35d0_sha_update_start(dd);
}

static void ma35d0_sha_dma_complete(struct nu_sha_reqctx *ctx)
{
	struct nu_sha_dev *dd = ctx->dd;
...
static int ma35d0_sha_update(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	...
	ma35d0_sha_sg_to_dma_buffer(req, ctx);
	if (ctx->bufcnt + ctx->req_len <= ctx->dma_max_size)
		return 0;
	return ma35d0_sha_handle_queue(ctx->dd, req);
}

static int ma35d0_sha_final(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int ma35d0_sha_finup(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	int err1, err2;
	 ...
	 */
	err2 = ma35d0_sha_final(req);

	return err1 ?: err2;
}

static int ma35d0_sha_digest(struct ahash_request *req)
{
	return ma35d0_sha_init(req) ?: ma35d0_sha_finup(req);
}

static int ma35d0_sha_setkey(struct crypto_ahash *tfm, const u8 *key,
			      u32 keylen)
{
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
...
static int ma35d0_sha_export(struct ahash_request *req, void *out)
{
...
static int ma35d0_sha_import(struct ahash_request *req, const void *in)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int ma35d0_sha_cra_init_alg(struct crypto_tfm *tfm,
				    const char *alg_base)
{
...
{
	.init		= ma35d0_sha_init,
	.update		= ma35d0_sha_update,
	.final		= ma35d0_sha_final,
	.finup		= ma35d0_sha_finup,
	.digest		= ma35d0_sha_digest,
	.export		= ma35d0_sha_export,
	.import		= ma35d0_sha_import,
	.halg.digestsize	= SHA1_DIGEST_SIZE,
	.halg.statesize = sizeof(struct nu_sha_reqctx),
	.halg.base	= {
		.cra_name		= "sha1",
		.cra_driver_name	= "ma35d0-sha1",
		.cra_priority		= 400,
		...
		.cra_init		= ma35d0_sha_cra_init,
		.cra_exit		= ma35d0_sha_cra_exit,
	}
},
{
	.init		= ma35d0_sha_init,
...
},
{
	...
	.export		= ma35d0_sha_export,
	.import		= ma35d0_sha_import,
	.setkey		= ma35d0_sha_setkey,
	.halg.digestsize	= SHA1_DIGEST_SIZE,
	.halg.statesize = sizeof(struct nu_sha_reqctx),
	.halg.base	= {
		.cra_name		= "hmac(sha1)",
		.cra_driver_name	= "ma35d0-hmac-sha1",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= SHA1_BLOCK_SIZE,
		.cra_ctxsize		= sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base	= {
		...
		.cra_driver_name	= "ma35d0-hmac-sha224",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= SHA224_BLOCK_SIZE,
		.cra_ctxsize		= sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base	= {
		...
		.cra_driver_name	= "ma35d0-hmac-sha256",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= SHA256_BLOCK_SIZE,
		.cra_ctxsize		= sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base	= {
		...
		.cra_driver_name	= "ma35d0-hmac-sha384",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= SHA384_BLOCK_SIZE,
		.cra_ctxsize		= sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base	= {
		...
		.cra_driver_name	= "ma35d0-hmac-sha512",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
		.cra_blocksize		= SHA512_BLOCK_SIZE,
		.cra_ctxsize		= sizeof(struct nu_sha_ctx),
...
static struct ahash_alg  ma35d0_sha3_algs[] = {
{
	.init		= ma35d0_sha_init,
	.update		= ma35d0_sha_update,
	.final		= ma35d0_sha_final,
	...
	.halg.base	= {
		.cra_name		= "sha3-224",
		.cra_driver_name	= "ma35d0-sha3-224",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
...
},
{
	...
	.halg.base	= {
		.cra_name		= "sha3-256",
		.cra_driver_name	= "ma35d0-sha3-256",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
...
},
{
	...
	.halg.base	= {
		.cra_name		= "sha3-384",
		.cra_driver_name	= "ma35d0-sha3-384",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
...
},
{
	...
	.halg.base	= {
		.cra_name		= "sha3-512",
		.cra_driver_name	= "ma35d0-sha3-512",
		.cra_priority		= 400,
		.cra_flags		= CRYPTO_ALG_ASYNC,
...
static void ma35d0_sha_queue_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;

	ma35d0_sha_handle_queue(dd, NULL);
}

/*
 *  This task is triggerred by Crypto IRQ when a SHA DMA completed.
 */
static void ma35d0_sha_done_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	int   map_size;
...
int ma35d0_sha_probe(struct device *dev,
		      struct nu_crypto_dev *nu_cryp_dev)
{
	...
	tasklet_init(&sha_dd->done_task, ma35d0_sha_done_task,
			(unsigned long)sha_dd);
	tasklet_init(&sha_dd->queue_task, ma35d0_sha_queue_task,
			(unsigned long)sha_dd);
	...
	for (i = 0; i < ARRAY_SIZE(ma35d0_sha_algs); i++) {
		err = crypto_register_ahash(&ma35d0_sha_algs[i]);
		if (err)
			goto err_register;
	}

	for (i = 0; i < ARRAY_SIZE(ma35d0_sha3_algs); i++) {
		err = crypto_register_ahash(&ma35d0_sha3_algs[i]);
		if (err)
			goto err_register;
	}
...
err_register:
	...
	for (i = 0; i < ARRAY_SIZE(ma35d0_sha_algs); i++)
		crypto_unregister_ahash(&ma35d0_sha_algs[i]);

	for (i = 0; i < ARRAY_SIZE(ma35d0_sha3_algs); i++)
		crypto_unregister_ahash(&ma35d0_sha3_algs[i]);
...
int ma35d0_sha_remove(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	struct nu_sha_dev  *sha_dd = &nu_cryp_dev->sha_dd;
	int	i;
