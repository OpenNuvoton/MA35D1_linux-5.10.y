...
static int ma35d0_sha_dma_run(struct nu_sha_dev *dd, int is_key_block)
{
	...
	if ((tctx->hash_mode & HMAC_CTL_SHA3EN) && (tctx->bufcnt == 0)) {
		/* workaround for MA35D0 SHA3 in case of DMACNT is 0 */
		ctx->reg_ctl |= HMAC_CTL_DMACSCAD;
	}
...
static void ma35d0_sha_sg_to_dma_buffer(struct ahash_request *req,
...
static int ma35d0_sha_update(struct ahash_request *req)
{
	...
	ma35d0_sha_sg_to_dma_buffer(req, tctx, ctx);
...
static int ma35d0_sha_setkey(struct crypto_ahash *tfm, const u8 *key, u32 keylen)
{
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
...
int ma35d0_sha_probe(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	...
	tasklet_init(&sha_dd->done_task, ma35d0_sha_done_task, (unsigned long)sha_dd);
	tasklet_init(&sha_dd->queue_task, ma35d0_sha_queue_task, (unsigned long)sha_dd);
	...
	pr_info("MA35D0 Crypto SHA engine enabled.\n");
	return 0;
...
int ma35d0_sha_remove(struct device *dev,
