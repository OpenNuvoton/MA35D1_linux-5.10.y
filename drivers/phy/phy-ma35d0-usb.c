...
struct ma35d0_usb_phy {
	struct device *dev;
	struct regmap *sysreg;
	struct clk *clk;
};

static int ma35d0_usb_phy_power_on(struct phy *phy)
{
	struct ma35d0_usb_phy *p_phy = phy_get_drvdata(phy);
...
static int ma35d0_usb_phy_power_off(struct phy *phy)
{
	struct ma35d0_usb_phy *p_phy = phy_get_drvdata(phy);
...
static const struct phy_ops ma35d0_usb_phy_ops = {
	.power_on = ma35d0_usb_phy_power_on,
	.power_off = ma35d0_usb_phy_power_off,
	.owner = THIS_MODULE,
};

static int ma35d0_usb_phy_probe(struct platform_device *pdev)
{
	struct ma35d0_usb_phy *p_phy;
	...
	phy = devm_phy_create(p_phy->dev, NULL, &ma35d0_usb_phy_ops);
	if (IS_ERR(phy)) {
...
static const struct of_device_id ma35d0_usb_phy_of_match[] = {
	{ .compatible = "nuvoton,ma35d0-usb-phy", },
	{ },
};
MODULE_DEVICE_TABLE(of, ma35d0_usb_phy_of_match);

static struct platform_driver ma35d0_usb_phy_driver = {
	.probe		= ma35d0_usb_phy_probe,
	.driver		= {
		.name	= "ma35d0-usb-phy",
		.of_match_table = ma35d0_usb_phy_of_match,
	},
};
module_platform_driver(ma35d0_usb_phy_driver);

MODULE_DESCRIPTION("Nuvoton ma35d0 USB2.0 PHY driver");
