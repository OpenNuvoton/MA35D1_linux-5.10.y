                             ...
                             int duty_ns, int period_ns)
{
	...
	if (ch == 0) {
		...
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT0);
	} else if (ch == 1) {
		__raw_writel(period - 1, ma35d1->regs + REG_PWM_PERIOD1);
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT1);
	} else if (ch == 2) {
		__raw_writel(period - 1, ma35d1->regs + REG_PWM_PERIOD2);
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT2);
	} else if (ch == 3) {
		__raw_writel(period - 1, ma35d1->regs + REG_PWM_PERIOD3);
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT3);
	} else if (ch == 4) {
		__raw_writel(period - 1, ma35d1->regs + REG_PWM_PERIOD4);
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT4);
	} else {/* ch 5 */
		__raw_writel(period - 1, ma35d1->regs + REG_PWM_PERIOD5);
		__raw_writel(duty, ma35d1->regs + REG_PWM_CMPDAT5);
	}
