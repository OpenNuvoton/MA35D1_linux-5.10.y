...
#define ma35h0_NUM_ROWS		8
#define ma35h0_NUM_COLS		8


struct ma35h0_keypad {
	struct clk *clk;
...
void ma35h0_keypad_mfp_set(struct platform_device *pdev)
{

}

static void ma35h0_keypad_scan_matrix(struct ma35h0_keypad *keypad,
										unsigned int status)
{
...
static irqreturn_t ma35h0_keypad_irq_handler(int irq, void *dev_id)
{
	struct ma35h0_keypad *keypad = dev_id;
	unsigned int  kstatus;

	kstatus = __raw_readl(keypad->mmio_base + KPI_STATUS);

	if (kstatus & (PKEY_INT|RKEY_INT)) {
		ma35h0_keypad_scan_matrix(keypad, kstatus);
...
static int ma35h0_keypad_open(struct input_dev *dev)
{
	struct ma35h0_keypad *keypad = input_get_drvdata(dev);
...
static void ma35h0_keypad_close(struct input_dev *dev)
{
	struct ma35h0_keypad *keypad = input_get_drvdata(dev);
...
static int ma35h0_keypad_probe(struct platform_device *pdev)
{
	struct ma35h0_keypad *keypad;
	struct input_dev *input_dev;
	struct resource *res;
	int irq;
	int error = 0;
	...
	input_dev->open = ma35h0_keypad_open;
	input_dev->close = ma35h0_keypad_close;
	...
	error = request_irq(keypad->irq, ma35h0_keypad_irq_handler,
...
static int ma35h0_keypad_remove(struct platform_device *pdev)
{
	struct ma35h0_keypad *keypad = platform_get_drvdata(pdev);
	struct resource *res;
...
static int ma35h0_keypad_suspend(struct platform_device *pdev,
									pm_message_t state)
{
	struct ma35h0_keypad *keypad = platform_get_drvdata(pdev);
...
static int ma35h0_keypad_resume(struct platform_device *pdev)
{
	struct ma35h0_keypad *keypad = platform_get_drvdata(pdev);
...
static const struct of_device_id ma35h0_kpi_of_match[] = {
	{ .compatible = "nuvoton,ma35h0-kpi"},
	{},
};
MODULE_DEVICE_TABLE(of, ma35h0_kpi_of_match);

static struct platform_driver ma35h0_keypad_driver = {
	.probe		= ma35h0_keypad_probe,
	.remove		= ma35h0_keypad_remove,
	.suspend	= ma35h0_keypad_suspend,
	.resume		= ma35h0_keypad_resume,
	.driver		= {
		.name	= "ma35h0-kpi",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(ma35h0_kpi_of_match),
	},
};
module_platform_driver(ma35h0_keypad_driver);

MODULE_AUTHOR("nuvoton");
MODULE_DESCRIPTION("ma35h0 keypad driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ma35h0-keypad");
