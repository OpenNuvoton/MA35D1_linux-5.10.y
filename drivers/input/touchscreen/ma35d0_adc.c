...
static int report_touch(struct ma35d0_adc *priv);
static void detect_touch(struct ma35d0_adc *priv);
static void detect_pendown(struct ma35d0_adc *priv);


static irqreturn_t ma35d0_adc_interrupt(int irq, void *private)
{
	struct ma35d0_adc *priv = (struct ma35d0_adc *)private;
	u32 isr, wkisr;
...
static void detect_penup(struct ma35d0_adc *priv)
{
	// Restart a new flow
	priv->pu_handle = 0;
...
static void ma35d0adc_ts_tasklet(struct ma35d0_adc *priv)
{
...
{
	struct ma35d0_adc *priv = container_of(hrtimer,
						struct ma35d0_adc,
						trigger_hrt);
	unsigned long flags;
...
static void detect_wakeup(struct ma35d0_adc *priv)
{
	unsigned long flags;
...
static void stop_detect(struct ma35d0_adc *priv)
{
	unsigned long flags;
...
static int ma35d0_ts_open(struct input_dev *dev)
{
	struct ma35d0_adc *priv = input_get_drvdata(dev);
...
static void ma35d0_ts_close(struct input_dev *dev)
{
	struct ma35d0_adc *priv = input_get_drvdata(dev);
...
static int ma35d0_ts_register(struct platform_device *pdev)
{
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
	struct input_dev *ts_dev;
	int ret;
	...
	if (!ts_dev)
		return -ENOMEM;
	ts_dev->name = "MA35D0 Touch Screen";
	ts_dev->phys = "ma35d0ts/event0";
	ts_dev->id.bustype = BUS_HOST;
	ts_dev->dev.parent = &pdev->dev;
	ts_dev->open = ma35d0_ts_open;
	ts_dev->close = ma35d0_ts_close;
...
static void ma35d0_ts_unregister(struct platform_device *pdev)
{
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
...
static void normal_convert(struct ma35d0_adc *priv, int ch)
{
	unsigned long flags;
...
static int ma35d0_adc_read_raw(struct iio_dev *idev,
			     struct iio_chan_spec const *chan,
			     int *val, int *val2, long mask)
{
	struct ma35d0_adc *priv = iio_device_get_drvdata(idev);
...
static const struct iio_info ma35d0_adc_info = {
	.read_raw = &ma35d0_adc_read_raw,
};

static int ma35d0_iio_register(struct platform_device *pdev)
{
	int ret, bit, idx = 0;
	struct iio_dev *indio_dev;
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
	...
	indio_dev->info = &ma35d0_adc_info;
...
static int ma35d0_iio_unregister(struct platform_device *pdev)
{
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
...
static int ma35d0_adc_probe_dt(struct ma35d0_adc *priv,
...
static int ma35d0_adc_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct ma35d0_adc *priv;
	int ret;

	dev_info(&pdev->dev, "Nuvoton MA35D0 ADC Driver\n");
	priv = devm_kzalloc(dev, sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	ret = ma35d0_adc_probe_dt(priv, pdev);
	if (ret) {
	...
	ret = devm_request_irq(&pdev->dev, priv->irq, ma35d0_adc_interrupt,
				0, dev_name(dev), priv);
	if (ret) {
	...
	if (priv->enable_ts) {
		ret = ma35d0_ts_register(pdev);
		if (ret)
			goto error_disable_clk;
	}

	if (priv->enable_iio) {
		ret = ma35d0_iio_register(pdev);
		if (ret)
...
static int ma35d0_adc_remove(struct platform_device *pdev)
{
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
...
static int ma35d0_adc_suspend(struct device *dev)
{
...
static int ma35d0_adc_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct ma35d0_adc *priv = platform_get_drvdata(pdev);
...
static SIMPLE_DEV_PM_OPS(ma35d0_adc_pm_ops,
		ma35d0_adc_suspend, ma35d0_adc_resume);

static const struct of_device_id ma35d0_adc_match[] = {
	{ .compatible = "nuvoton,ma35d0-adc" },
	{},
};

static struct platform_driver ma35d0_adc_driver = {
	.probe = ma35d0_adc_probe,
	.remove = ma35d0_adc_remove,
	.driver = {
		.name = "ma35d0-adc",
		.of_match_table = ma35d0_adc_match,
		.pm = &ma35d0_adc_pm_ops,
	},
};

module_platform_driver(ma35d0_adc_driver);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Nuvoton MA35D0 ADC Driver");
