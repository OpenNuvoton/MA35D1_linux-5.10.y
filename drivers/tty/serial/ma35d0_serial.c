...
static void ma35d0serial_console_putchar(struct uart_port *port, int ch)
{
	struct uart_ma35d0_port *up = (struct uart_ma35d0_port *)port;
...
#define MA35D00SERIAL_CONSOLE    (&ma35d0serial_console)
#else
#define MA35D00SERIAL_CONSOLE    NULL
#endif
...
static int ma35d0serial_remove(struct platform_device *dev)
{
	...
	for (i = 0; i < UART_NR; i++) {
		struct uart_ma35d0_port *up = &ma35d0serial_ports[i];
